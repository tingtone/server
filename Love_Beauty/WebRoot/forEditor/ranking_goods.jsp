...
<html>
  <head>
	...
	function GetRankingGoods(rid){
			var url=BASE_SERVER+"/ranking_getRankingGoods";
			var params = {"rid":rid};
			var rankingGoods="";
			var goodsDetail="";
			$.ajax({
				...
				if(json == null){
					...
					for(var i=0;i<json.length;i++){
						...
						rankingGoods+="<td><input type='hidden' name='id' value='"+json[i]['id']+"'/><input type='hidden' name='rid' value='"+$('#ranking_id').val()+"'/><input type='hidden' name='ranking_name' value='"+$('#ranking_name').text()+"'/></td>";
						if(i==0&&json.length!=1){rankingGoods+="<td>商品号：<input type='hidden' name='id_exchange' value='"+json[i+1]['id']+"'/><input type='text' name='gid' value='"+goodsId+"'/>商品名: <input name='goods_name' value='"+json[i]['goods']['goodsName']+"'/></td><td><input name='submit' type='submit' value='后移'/><input name='submit' type='submit' value='删除'/><input type='button' gid='"+goodsId+"' onclick=\"$('#goods"+goodsId+"').show();\" value='显示商品详情' id='showGoods'/></td>" ;
						}
